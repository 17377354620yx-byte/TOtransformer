...
usage() {
  cat <<'EOF'
Usage: bash scripts/run_p2p_togg_full.sh train|test|all [TRAIN_ARGS...]

Examples:
  bash scripts/run_p2p_togg_full.sh all
  bash scripts/run_p2p_togg_full.sh train
  bash scripts/run_p2p_togg_full.sh train --resume
  bash scripts/run_p2p_togg_full.sh test
...
Environment overrides:
  ...
  P2P_SNAPSHOT=/path/to/checkpoint.pth.tar \
    bash scripts/run_p2p_togg_full.sh test
EOF
}
